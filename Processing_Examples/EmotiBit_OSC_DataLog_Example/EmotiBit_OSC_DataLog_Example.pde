...
String dataSaveDir;
String[] oscAddresses = {"/EmotiBit/0/EDA", "/EmotiBit/0/SCR:AMP"}; 
//String[] oscAddresses = {"/EmotiBit/0/PPG:IR", "/EmotiBit/0/PPG:RED"}; 
String plotOscAddress = oscAddresses[0];
int oscPort = 12345;
...
FileWriter[] outputFiles = new FileWriter[oscAddresses.length];
String[] filenames = new String[oscAddresses.length];
...
void setup() {
  ...
  dataSaveDir = sketchPath("");
  
  String datestamp = year() + "-" + month() + "-" + day() + "_" + hour() + "-" + minute() + "-" + second();

  for (int i=0; i<oscAddresses.length; i++) {
    String oscAddress = oscAddresses[i];    
    String filename = oscAddress.replace("/","").replace(":","");
    filenames[i] = filename;
    try {
      outputFiles[i] = new FileWriter(dataSaveDir + datestamp + "_" + filename + ".csv", true);
    }
    catch(IOException e) {
      println("It broke!!!");
      e.printStackTrace();
  ...
  for (int i=0; i<oscAddresses.length; i++) {
    ...
    if (theOscMessage.checkAddrPattern(oscAddress)) {
      ...
      for (int n = 0; n < args.length; n++)
      {
        ...
        try {
          outputFiles[i].write("" + d.getTime() + ", " + nf(data, 0, 8) + "\n");
          outputFiles[i].flush();
          //outputFiles[i].close();
        }
        catch(IOException e) {
